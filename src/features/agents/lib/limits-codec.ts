 ...
 *
 * Ported verbatim from the MCP (`@ton/mcp` `src/limits/limits-codec.ts`) so the
 * dashboard (the only writer of limits) serializes byte-for-byte what the MCP
 * reader/verifier expects. The contract never parses or stores this dictionary:
 * limits are carried in the `ChangeNftContentMsg` (opcode 0x1a0b9d51) body and
 * recovered off-chain. The integrity anchor is `limits_hash` = the
 * cell-representation hash of `beginCell().storeDictDirect(limitsDict).endCell()`.
 */
